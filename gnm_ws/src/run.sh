...
setup() {
    ...
    local sleep_time=$3
    echo "source ~/miniconda3/etc/profile.d/conda.sh && conda activate $env_name && source /opt/ros/humble/setup.bash && source install/setup.bash && echo 'starting $session_name...' && sleep $sleep_time"
}
